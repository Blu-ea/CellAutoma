...
const vec4 subcell_colour = vec4(  0.1,  0.0,  0.0, 0.05 );
...
void main() {
    if (any(lessThan(distance_to_cell, vec2(adjusted_cell_line_thickness * 0.5))))
        outColor = cell_colour;
    else if (any(lessThan(distance_to_subcell, vec2(adjusted_subcell_line_thickness * 0.5))))
        outColor = subcell_colour;
